...
function freegame($id) {
	...
	break;		case "opengame":        $content = freegame_opengame();	break;
    case "scorecard":
        /*$id = htmlspecialchars($_GET["id"]);*/
        $holeid = htmlspecialchars($_GET["holeid"]);
...
function freegame_menu($id) {
    ...
    return $content;
}function freegame_opengame() {	$sql = 'SELECT `id`,`timestamp` FROM `sc_fg_games`			ORDER BY `timestamp` DESC			LIMIT 0,10';    $result = db_query($sql);		while( $row = mysqli_fetch_assoc($result)) {		$content .= '<a href="?op=freegame&action=hole&id='.$row["id"].'"><div class="button">'.$row["id"].' ('.date_format(date_create($row["timestamp"]),'d.m.Y / H:i').')</div></a>';    }        return $content;}
...
function freegame_newedit() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $id = $formdata[id];
        if (!$id) { do {$id = rand(100000,999999);} while (check_fg_id($id)==0); }
